<!-- check about priorityTask and date valedation  -->
<?php
session_start();
...
else
{
    ...
    $checkTaskName="SELECT TaskName FROM usertasks WHERE TaskName='$taskName' AND userName='$userName'";
    $result=$con->query($checkTaskName);

    if($result->num_rows>0)
    {
        $errorMessaga="*TaskName Can't Be Repeated !!  "; 

    }
    else
    {
            $insertTask="INSERT INTO usertasks (taskName,taskExpirationDate,priorityTask,taskCatagory,taskDescription,userName)
            VALUES('$taskName','$taskExDate','$priorityTask','$taskCatagory','$description','$userName')";
            if($con->query($insertTask)===true)
            {
                $errorMessaga="*Add Task Successfully "; 
            }
    }}
...
Add Task  
            ...
            <label for="taskExDate"> Task Expiration Date	</label>
            <input type="date" id="taskExDate" name="taskExDate" required>

            <label for="taskCatagory"> Task Catagory</label>
            ...
            <label for="priorityTask"> Task Priority : </label>
            <input type="text" name="priorityTask" id="priorityTask" placeholder="Critical / Minor " required>
        
            </div>
            <div class="form-group">
            <label for="description">Task Description</label>
            <textarea name="description" id="description" rows="10" cols="50" required>
            </textarea>
            </div>
            ...
            if (isset($_POST['submit']) && $result->num_rows > 0)
			      {
                echo "<p style='color:#ffc400;font-size:12px;padding-top:7px'>".$errorMessaga."</p>";
            }
            ...
            <button class="form-submit-btn" type="submit" name="submit">ADD</button>
        </form>
    </div>
</div>
<!-- END Add task  -->
